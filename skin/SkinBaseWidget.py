...
from Ui_ScrollArea import Ui_FormScrollArea
import os

PixmapWidth=158  #图片的宽度
PixmapHeight=152 #图片的高度
...
class SkinBaseItemWidget(QWidget):
    ...
    def __init__(self,name,colorimg,signal,*args,**kwargs):
        ...
        if isinstance(self.image,str) and os.path.isfile(self.image):
            #IgnoreAspectRatio 根据图片自身大小缩放
            #SmoothTransformation 平滑缩放
            self.image=QPixmap(self.colorimg).scaled(
                PixmapWidth,PixmapHeight,Qt.IgnoreAspectRatio,Qt.SmoothTransformation
            )
    ...
    def mouseReleaseEvent(self,event):
        ...
        self.textColor=QColor(102,102,102)
        self.update()
        self.signal.emit(self.name,self.colorCount)
    ...
    def paintEvent(self,event):
        ...
        painter.drawText(0,0,PixmapWidth,PixmapHeight+MarginBottom,Qt.AlignHCenter|Qt.AlignBottom,self.name)
        painter.end()
